...
//IS BASED FROM test/simple_strategy_testing/test.js
...
function select_indicators_results(indicatorsData, lastTimestampNeed){
  var selectIndicatorsData={};
  var timestamp=Object.keys(indicatorsData)[0];
  var i=0;

  while(parseInt(timestamp)<=parseInt(lastTimestampNeed)){
    selectIndicatorsData[timestamp]=indicatorsData[timestamp];
    timestamp=Object.keys(indicatorsData)[i];
    i++;
  }
  return selectIndicatorsData;
}
...
exports.start = function(strat_file, quotfile, onSuccess, onError){
  strat=require(strat_file);

  OpenJSONquotation.start(Fs,quotfile,function(instru_list_check, instru_quotation){
...
//        Ld.update(ldProgress);
  ...
  //    console.log(Object.keys(instru_quotation.stock[instru]).length+" - "+Object.keys(allIndiData).length);
      ...
      var lastData=select_quot(instru_quotation.stock[instru],Object.keys(instru_quotation.stock[instru]).length);
      var completeIndiResults=AutomaticIndicatorCalculation.start(instru, lastData, idctr_global_settings, strat.specificalIndicatorsSettings);

      var date1 = new Date();
      var date2 =0;
      var date4 =0;

      var indiResults=select_indicators_results(completeIndiResults,Object.keys(instru_quotation.stock[instru])[strat.global_settings.minimum_of_data]);
...
//          Ld.update(ldProgress);
    ...
    //    console.log(b+"/"+(Object.keys(instru_quotation.stock[instru]).length-1));
        ...
        //ADD NEW DATA AT EVERY TIMESTAMP
        lastData[actualTimestamp]=instru_quotation.stock[instru][actualTimestamp];
        ...
        //ADD NEW INDI DATA AT EVERY TIMESTAMP
        indiResults[actualTimestamp]=completeIndiResults[actualTimestamp];
